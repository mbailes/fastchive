...
int dialog::addbutton(float xx, float yy, const char * llabel){
...
int dialog::addradio(float xx, float yy, const char *llabel, int status, int group){
  
  if (nradio>NELEMENTS) {
    fprintf(stderr,"Too many radios in dialog nradio = %d max = %d\n",nradio,NELEMENTS);
    exit(-1);
  }
  radios[nradio].x = xx;
  radios[nradio].y = yy;
...
int dialog::addcheck(float xx, float yy, const char *llabel, int status){
...
int dialog::addstaticText(float xx, float yy, const char *llabel, int c,
			  float s, float o){
...
button::button(float xi, float yi, const char *l){
x=xi;
y=yi;
...
check::check(float xx, float yy, const char *l, int status){
  x=xx; y=yy; on=status;
  xmin = xx, ymin = yy;
...
radio::radio(float xx, float yy, const char *l, int status, int group_input){
  x = xx; y = yy; label = new char[strlen(l)+1]; strcpy(label,l);
  on=status; groupid = group_input;
}

int plotregion::insideworld(float x, float y){
  if ((x>=xminworld) && (x<=xmaxworld) && 
      (y>=yminworld) && (y<=ymaxworld)) return(1);
  else
    return(0);
}
...
staticText::staticText(float xx, float yy, const char *input, int colour, float ss,
		       float o){
